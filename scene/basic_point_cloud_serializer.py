...
class BasicPointCloudSerializer:
    ...
    def save_to_ply(data: BasicPointCloud,
        ...
        normals = data.normals if data.normals else np.zeros_like(data.points)
        colors = np.clip(data.colors * 255.0, a_min=0.0, a_max=255.0).astype(np.uint8)

        elements = np.empty(data.points.shape[0], dtype=dtype)
        attributes = np.hstack((data.points, normals, colors))
        elements[:] = list(map(tuple, attributes))

        vertex_element = PlyElement.describe(data=elements, name="vertex")
