...
class GaussianModel:
    ...
    def create_from_pcd(self,
        ...
        fused_point_cloud = torch.tensor(np.asarray(pcd.positions)).float().cuda()
        ...
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.positions)).float().cuda()), 0.0000001)
        ...
        self.max_radii_2d = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self.exposure_mapping = {cam_info.image_file_name: idx for idx, cam_info in enumerate(cam_infos)}
